...
public class MarshalTest extends CamelQuarkusTestSupport {
    ...
    protected RouteBuilder createRouteBuilder() {
        return new RouteBuilder() {
            public void configure() throws Exception {
            	...
            	JacksonDataFormat orderToProducerFormat = new JacksonDataFormat(OrderToProducer.class);
            	
                from("direct:start" + TEST_NAME)
                	.log("start")
                	.unmarshal(orderToProducerFormat)
                	.process(new Processor() {
                		public void process(Exchange exchange) throws Exception {
                			exchange.getIn().getBody(OrderToProducer.class).setAmount(111);
                		};
                	})
                	.marshal(orderToProducerFormat)
                	.log("end")
                	.to(resultEndpoint);
    ...
    @Test
    public void test_marshall_with_json_and_prettyprint() throws InterruptedException {
        String msgIn = ExampleFiles.ORDER_TO_PRODUCER_EGGS_JSON;
        String msgExpected = ExampleFiles.ORDER_TO_PRODUCER_111_EGGS_JSON;
